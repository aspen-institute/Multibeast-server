...
/* view all route for filtering */

router.get('/api/v1/all/:filter=:value', async (req, res) => {	
	let searchParam = req.params.value;
	let searchKey;
	let sort;
	if (req.params.filter === 'location') {
		searchKey = 'ArtsVisionFork.SessionLocation';
		sort = {
			'ArtsVisionFork.SessionDate':1,
			'ArtsVisionFork.StartTime':1
		}
	} else if (req.params.filter === 'date') {
		searchKey = 'ArtsVisionFork.SessionDate';
		sort = {
			'ArtsVisionFork.StartTime':1
		}
	}
